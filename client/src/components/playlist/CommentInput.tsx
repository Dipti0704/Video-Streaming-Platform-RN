...
interface CommentInputProps extends SocketProps {
  animeId: string;
}

const CommentInput: FC<CommentInputProps> = ({ initialValue, setInitialValue, animeId }) => {
  ...
  const handleSendComment = () => {
    if (value.trim()) {
      socketService.emit('NEW_COMMENT', { animeId: animeId, comment: value });
      setValue('');
    }
  };
